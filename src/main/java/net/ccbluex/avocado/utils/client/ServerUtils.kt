...
object ServerUtils : MinecraftInstance {
    ...
    fun connectToLastServer(noGLContext: Boolean = false) {
        ...
        if (noGLContext) {
            SharedScopes.IO.launch {
                ...
                networkManager.netHandler = NetHandlerLoginClient(networkManager, mc, net.minecraft.client.gui.GuiMainMenu())
        ...
        } else mc.displayGuiScreen(GuiConnecting(GuiMultiplayer(net.minecraft.client.gui.GuiMainMenu()), mc, serverData))
    }
    ...
    fun hideSensitiveInformation(address: String): String {
        return if (address.contains(".avocado.net")) {
            "<redacted>.avocado.net"
