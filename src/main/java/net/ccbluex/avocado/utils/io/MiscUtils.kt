...
object MiscUtils : MinecraftInstance {
    ...
    fun generateCrashInfo(): String {
        var base = """
            --- Game crash info ---
            Client: ${Avocado.CLIENT_NAME} ${Avocado.clientVersionText} (${Avocado.clientCommit})
    ...
    fun Throwable.showErrorPopup(
        ...
        val openIssueButton = JButton("Open GitHub Issue").apply {
            addActionListener {
                showURL("${Avocado.CLIENT_GITHUB}/issues/new?template=bug_report.yml&title=%5BBUG%5D+Game+crashed+$exceptionType")
            }
        }
